...
from magenta.models.music_vae.student_teacher_model import StudentTeacher
...
def train(run_dir,
  ...
  if teacher_ckpt_no >= 0:
    teacher_checkpoint_path = os.path.join(run_dir, 'train/model.ckpt-{}'.format(teacher_ckpt_no))
  else:
    teacher_checkpoint_path = BASE_DIR + '/checkpoints/' + config_name[4:] + '.ckpt'

  if student_ckpt_no >= 0:
    student_checkpoint_path = os.path.join(run_dir, 'train/model.ckpt-{}'.format(student_ckpt_no))
  else:
    student_checkpoint_path = BASE_DIR + '/checkpoints/' + config_name[4:] + '.ckpt'
  ...
  with tf.Graph().as_default():
    with tf.device(tf.train.replica_device_setter(
      ...
      print("=================================================")
      print("==================INFO==================")
      print("Alpha is: ", config.hparams.alpha)
      print("Variables are...")
      ...
      teacher_variables_to_restore = [v for v in 
      tf.get_collection_ref(tf.GraphKeys.GLOBAL_VARIABLES) 
                            if v.name!='global_step:0' 
                            and v.name[:7]=='teacher']
      # create variable name mapping for teacher
      teacher_var_dict = {v.name[8:-2]:v for v in teacher_variables_to_restore}

      if finetune:
        student_variables_to_restore = [v for v in 
        tf.get_collection_ref(tf.GraphKeys.GLOBAL_VARIABLES) 
                              if v.name!='global_step:0' 
                              and v.name[:7]=='student']
        # create variable name mapping for student
        student_var_dict = {v.name[8:-2]:v for v in student_variables_to_restore}

      from tf_slim.ops.variables import assign_from_checkpoint
      teacher_init_op, teacher_feed_dict = assign_from_checkpoint(teacher_checkpoint_path,
                                  teacher_var_dict)
      if finetune:
        student_init_op, student_feed_dict = assign_from_checkpoint(student_checkpoint_path,
                                    student_var_dict)
      def init_fn(sess):
        sess.run(teacher_init_op, teacher_feed_dict)
        if finetune:
          sess.run(student_init_op, student_feed_dict)
...
def evaluate(run_dir,
  ...
  with tf.Graph().as_default():
    ...
    # variables_to_restore = [v for v in 
    # tf.get_collection_ref(tf.GraphKeys.GLOBAL_VARIABLES) if v.name=='global_step:0' or v.name[:7]=='student']
    # create variable name mapping for teacher
    # var_dict = {v.name[8:-2]:v for v in variables_to_restore}

    # from tf_slim.ops.variables import assign_from_checkpoint_fn
    # init_fn = assign_from_checkpoint_fn(checkpoint_path, var_dict) 
    ...
    #method_one via tensorflow
    # from tensorflow.python.training.evaluation import _evaluate_once
    # _evaluate_once(
    #   logdir=eval_dir,
    #   scaffold=scaffold,
    #   master=master,
